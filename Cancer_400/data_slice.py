...
## 文件夹地址
dir = "data/"
input_pth = dir + "Cancer_Metabolism.txt"
info_pth = dir + "Cancer400_info.txt"

##读取文件中总数据集
with open(input_pth, newline='', encoding='utf-8') as in_file:
...
## 按照TSV格式写入
with open(dir + 'TrainingSet.txt', 'wt', newline='', encoding='utf-8') as train_file:
    tsv_writer = csv.writer(train_file, delimiter='\t')
    tsv_writer.writerows(Meta_train)
train_file.close()

with open(dir + 'TestSet.txt', 'wt', newline='', encoding='utf-8') as test_file:
    info_writer = csv.writer(test_file, delimiter='\t')
    info_writer.writerows(Meta_test)
test_file.close()

with open(dir + 'EvaluationSet.txt', 'wt', newline='', encoding='utf-8') as eva_file:
...
Meta_info(Meta_class, train_class, test_class, eva_class, info_pth)